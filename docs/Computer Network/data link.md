...
每个帧的开始和结束是标志字节：01111110（Ox7E）
  ...
  - 接收方：看到 5 个连续比特 1 ，后面跟一个比特0，它就自动剔除（即删除）比特 0 。
...
- 保证 n 位纠错码能够表示上述所有不同的码字：$2^m \cdot \Sigma_{k = 0}^d\C_n^k \leq 2^n$。
...
-  其实相当于异或运算的结果！
- 倒序写各个异或运算的结果，即得到出错的1比特的序号！（原理就是每个校验位对应了比特位序号的二进制中的1位）
...
- 接收到帧，检查目的 MAC 地址，决定转发到哪一接口，或者丢弃
- 分割冲突域
    - 过滤通信量；增大吞吐量（速率总和?）
    - 提高可靠性（不同冲突域不互相影响）
